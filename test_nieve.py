from naive import merge_sort_python
import random
import unittest
...
class Tests(unittest.TestCase):
    ...
    def test_empty(self):
        empty = []
        self.assertEqual(merge_sort_python(empty), empty)

    def test_one(self):
        one = [1]
        self.assertEqual(merge_sort_python(one), one)

    def test_two_in_order(self):
        two = [1,9]
        self.assertEqual(merge_sort_python(two), sorted(two))
    ...
    def test_three_abc(self):
        abc = [1,2,3]
        self.assertEqual(merge_sort_python(abc), sorted(abc))
    ...
    def test_16(self):
        ...
        self.assertEqual(merge_sort_python(st), sorted(st))
    ...
    def test_15(self):
        ...
        self.assertEqual(merge_sort_python(ft), sorted(ft))

    def test_100(self):
        hunnid = [random.randrange(100) for i in range(100)]
        self.assertEqual(merge_sort_python(hunnid), sorted(hunnid))

    def test_1024(self):
        ttf = [random.randrange(100) for i in range(1024)]
        self.assertEqual(merge_sort_python(ttf), sorted(ttf))

    def test_1023(self):
        ttt = [random.randrange(100) for i in range(1023)]
        self.assertEqual(merge_sort_python(ttt), sorted(ttt))
